...
from analyzer.utils import xyxys_in_xyxys, find_static_file, fetch_pickle
...
def tensor_to_stat(tensor: torch.tensor):
    stats = torch.round(tensor)
    return dict((t, int(s)) for t, s in zip(STAT_TYPE, stats))
...
class ModelEvaluator:
    ...
    def stat_evaluater(boxes_list_categorized: InputDataType):
        ...
        normalizer_path = f'model/normalizer.pkl'
        model_path = f'model/stat_evaluator.pt'
