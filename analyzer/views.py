...
def get_wh(box: BoundingBoxType):
    _, _, w, h = map(int, box.xyxy[0])
    return w, h


def get_area(box: BoundingBoxType):
    w, h = get_wh(box)
    return w * h


def get_xyxys(boxes: List[BoundingBoxType]):
    return map(lambda box: tuple(map(int, box.xyxy[0])), boxes)


def get_whs(boxes: List[BoundingBoxType]):
    return map(lambda box: tuple(map(int, box.xywh[0]))[2:], boxes)


def get_areas(boxes: List[BoundingBoxType]):
    whs = get_whs(boxes)
...
class BoxData:
    ...
    @property
    def _xyxys(self):
        return get_xyxys(self._boxes)

    @property
    def _whs(self):
        return get_whs(self._boxes)
...
class ImagesContainer:
    def __init__(self, images: List[Path]):
        container = dict()
        for category, path in zip(DATA_CATEGORY, images):
            container[category] = path
...
def stat_evaluater(boxes_list_categorized: Tuple[List[BoundingBoxType]]):
    boxes_list_list = [boxes_list for boxes_list in zip(*boxes_list_categorized)]
    scores = [stat_evaluater_single(boxes_list) for boxes_list in boxes_list_list]
    return scores
...
def analyzer(images_list: Union[Tuple[Path], List[Tuple[Path]]]):
    ...
    total_score = stat_evaluater(boxes_list_categorized)
    return total_score
