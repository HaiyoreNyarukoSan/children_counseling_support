...
def signup_patient(request):
    if request.method == "POST":
        ...
        formset = patient_form_set(request.POST, prefix="patientForm")
        ...
        formset = patient_new_form_set(
            queryset=Patient.objects.none(), prefix="patientForm"
        )
    empty_form = formset.empty_form
    empty_form.prefix = "patientForm-__prefix__"
    context = {"form": form, "formset": formset, "empty_form": empty_form}
...
def change(request):
    return render(request, "users/choose_your_change.html")
...
def change_information(request):
    if request.method == "POST":
        ...
        form_counselor = (
            CounselorSignUpForm(
                data=request.POST, files=request.FILES, instance=counselor
            )
            if counselor
            else None
        )
        if _validate_forms(form, formset, form_counselor):
            ...
            if formset:
                saved_formset = formset.save(commit=False)
                for saved_patient in saved_formset:
                    saved_patient.p_user = request.user
                    saved_patient.save()
            if form_counselor:
                form_counselor.save()
            return redirect("users:My-Page")
        ...
        patients = Patient.objects.filter(p_user=request.user)
        formset = (
            patient_form_set(queryset=patients, prefix="patientForm")
            if patients.exists()
            else None
        )
        counselor = getattr(request.user, "counselor", None)
